package cn.bcd.app.bp.backend.sys.keys;
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        if (KeysConst.IS_CLUSTER) {
            ...
            if (keys == null) {
                ...
                keys = new String[2];
                keys[0] = Base64.getEncoder().encodeToString(rsaKeyPair.getPublicKeyBytes());
                keys[1] = Base64.getEncoder().encodeToString(rsaKeyPair.getPrivateKeyBytes());
            ...
            KeysConst.PUBLIC_KEY = RSAUtil.publicKeyFromBytes(Base64.getDecoder().decode(keys[0]));
            KeysConst.PRIVATE_KEY = RSAUtil.privateKeyFromBytes(Base64.getDecoder().decode(keys[1]));
