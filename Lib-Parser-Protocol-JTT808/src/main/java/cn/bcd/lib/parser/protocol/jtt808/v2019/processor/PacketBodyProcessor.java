...
public class PacketBodyProcessor implements Processor<PacketBody> {
    ...
    public PacketBody process(ByteBuf data, ProcessContext processContext) {
        PacketHeader packetHeader = (PacketHeader) processContext.getGlobalVar("packetHeader");
        ...
        switch (packetHeader.msgId) {
            ...
            case terminal_register_request -> {
                packetBody = TerminalRegisterRequest.read(data, packetHeader.msgLen);
            }
            case terminal_register_response -> {
                packetBody = TerminalRegisterResponse.read(data, packetHeader.msgLen);
            }
            ...
            case terminal_control -> {
                packetBody = TerminalControl.read(data, packetHeader.msgLen);
            }
            ...
            case position_data_upload -> {
                packetBody = Position.read(data, packetHeader.msgLen);
            }
            case query_position_response -> {
                packetBody = QueryPositionResponse.read(data, packetHeader.msgLen);
            }
            ...
            case text_info_issued -> {
                packetBody = TextInfoIssued.read(data, packetHeader.msgLen);
            }
            case phone_callback -> {
                packetBody = PhoneCallback.read(data, packetHeader.msgLen);
            }
            case set_phone_text -> {
            ...
            case vehicle_control_response -> {
                packetBody = VehicleControlResponse.read(data, packetHeader.msgLen);
            }
            ...
            case driving_recorder_collect_command, driving_recorder_download_command -> {
                packetBody = DrivingRecorderCollectCommand.read(data, packetHeader.msgLen);
            }
            case driving_recorder_upload -> {
                packetBody = DrivingRecorderUpload.read(data, packetHeader.msgLen);
            }
            case waybill_report -> {
            ...
            case multimedia_data_upload_request -> {
                packetBody = MultimediaDataUploadRequest.read(data, packetHeader.msgLen);
            }
            case multimedia_data_upload_response -> {
                packetBody = MultimediaDataUploadResponse.read(data, packetHeader.msgLen);
            }
            ...
            case storage_multimedia_data_fetch_response -> {
                packetBody = StorageMultimediaDataFetchResponse.read(data, packetHeader.msgLen);
            }
            ...
            case data_down_stream -> {
                packetBody = DataDownStream.read(data, packetHeader.msgLen);
            }
            case data_up_stream -> {
                packetBody = DataUpStream.read(data, packetHeader.msgLen);
            }
            case data_compress_report -> {
                packetBody = DataCompressReport.read(data, packetHeader.msgLen);
            }
            case platform_rsa -> {
            ...
            default -> throw BaseException.get("msgId[{}] not support", packetHeader.msgId);
        }
        return packetBody;
    }
