...
public final class FieldLog__F_customize extends FieldLog<F_customize> {
    static Logger logger = LoggerFactory.getLogger(FieldLog__F_customize.class);

    public FieldLog__F_customize() {
        super(F_customize.class);
    }
    ...
    /**
     * 输出 @F_customize 字段的解析日志；每次字段解析只打印一行。
     *
    ...
    public static void parse(Class<?> clazz, String fieldName, byte[] content,
        ...
        logger.info("--parse @F_customize field{}--[{}.{}] hex[{}] val{}", location, clazz.getSimpleName(),
                fieldName, hex, LogUtil.formatLogValue(value));
    }

    /**
     * 输出 @F_customize 字段的反解析日志；每次字段反解析只打印一行。
     *
     * @param clazz        字段所属的模型类
     * @param fieldName    字段名称
    ...
    public static void deParse(Class<?> clazz, String fieldName, byte[] content,
        ...
        String location = LogUtil.getFieldLocation(clazz, fieldName);
        String hex = ByteBufUtil.hexDump(content).toUpperCase();
        logger.info("--deParse @F_customize field{}--[{}.{}] val{} hex[{}]", location, clazz.getSimpleName(),
