...
public final class FieldLog__F_string_bcd extends FieldLog<F_string_bcd> {
    static Logger logger = LoggerFactory.getLogger(FieldLog__F_string_bcd.class);

    public FieldLog__F_string_bcd() {
        super(F_string_bcd.class);
    }

    @Override
    public void buildParseAfter(BuilderContext context) {
        F_string_bcd annotation = context.field.getAnnotation(F_string_bcd.class);
    ...
    /**
     * 输出 @F_string_bcd 字段的解析日志；每次字段解析只打印一行。
     *
    ...
    public static void parse(Class<?> clazz, String fieldName, byte[] content,
        ...
        if (skipped) {
            logger.info("--parse @F_string_bcd skip field{}--[{}.{}] hex[{}]", location, clazz.getSimpleName(), fieldName, hex);
        } else {
            logger.info("--parse @F_string_bcd field{}--[{}.{}] hex[{}] val{}", location, clazz.getSimpleName(),
                    fieldName, hex, LogUtil.formatLogValue(value));
        }
    }

    /**
     * 输出 @F_string_bcd 字段的反解析日志；每次字段反解析只打印一行。
     *
     * @param clazz        字段所属的模型类
     * @param fieldName    字段名称
    ...
    public static void deParse(Class<?> clazz, String fieldName, byte[] content,
        ...
        String hex = ByteBufUtil.hexDump(content).toUpperCase();
        if (skipped) {
            logger.info("--deParse @F_string_bcd skip field{}--[{}.{}] hex[{}]", location, clazz.getSimpleName(), fieldName, hex);
        } else {
            logger.info("--deParse @F_string_bcd field{}--[{}.{}] val{} hex[{}]", location, clazz.getSimpleName(),
