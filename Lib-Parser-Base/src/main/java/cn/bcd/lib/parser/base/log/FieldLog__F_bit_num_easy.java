...
public final class FieldLog__F_bit_num_easy extends FieldLog<F_bit_num_easy> {
    static Logger logger = LoggerFactory.getLogger(FieldLog__F_bit_num_easy.class);

    public FieldLog__F_bit_num_easy() {
        super(F_bit_num_easy.class);
    }

    @Override
    public void buildParseAfter(BuilderContext context) {
        F_bit_num_easy annotation = context.field.getAnnotation(F_bit_num_easy.class);
    ...
    /**
     * 输出 @F_bit_num_easy 字段的解析日志；每次字段解析只打印一行。
     *
    ...
    public static void parse(Class<?> clazz, String fieldName, byte[] content,
        ...
        if (skipped) {
            logger.info("--parse @F_bit_num_easy skip field{}--[{}.{}] hex[{}]", location, clazz.getSimpleName(), fieldName, hex);
        } else {
            logger.info("--parse @F_bit_num_easy field{}--[{}.{}] hex[{}] val{}", location, clazz.getSimpleName(),
                    fieldName, hex, LogUtil.formatLogValue(value));
        }
    }

    /**
     * 输出 @F_bit_num_easy 字段的反解析日志；每次字段反解析只打印一行。
     *
     * @param clazz        字段所属的模型类
     * @param fieldName    字段名称
    ...
    public static void deParse(Class<?> clazz, String fieldName, byte[] content,
        ...
        String hex = ByteBufUtil.hexDump(content).toUpperCase();
        if (skipped) {
            logger.info("--deParse @F_bit_num_easy skip field{}--[{}.{}] hex[{}]", location, clazz.getSimpleName(), fieldName, hex);
        } else {
            logger.info("--deParse @F_bit_num_easy field{}--[{}.{}] val{} hex[{}]", location, clazz.getSimpleName(),
