...
public final class FieldLog__F_bit_num extends FieldLog<F_bit_num> {
    static Logger logger = LoggerFactory.getLogger(FieldLog__F_bit_num.class);

    public FieldLog__F_bit_num() {
        super(F_bit_num.class);
    }

    @Override
    public void buildParseAfter(BuilderContext context) {
        F_bit_num annotation = context.field.getAnnotation(F_bit_num.class);
    ...
    /**
     * 输出 @F_bit_num 字段的解析日志；每次字段解析只打印一行。
     *
    ...
    public static void parse(Class<?> clazz, String fieldName, byte[] content,
        ...
        if (skipped) {
            logger.info("--parse @F_bit_num skip field{}--[{}.{}] hex[{}] bit[{}]", location, clazz.getSimpleName(), fieldName, hex, detail);
        } else {
            logger.info("--parse @F_bit_num field{}--[{}.{}] hex[{}] val{} bit[{}]", location, clazz.getSimpleName(),
                    fieldName, hex, LogUtil.formatLogValue(value), detail);
        }
    }

    /**
     * 输出 @F_bit_num 字段的反解析日志；每次字段反解析只打印一行。
     *
     * @param clazz        字段所属的模型类
     * @param fieldName    字段名称
    ...
    public static void deParse(Class<?> clazz, String fieldName, byte[] content,
        ...
        String detail = LogUtil.formatBitLogs(bitLogs);
        if (skipped) {
            logger.info("--deParse @F_bit_num skip field{}--[{}.{}] hex[{}] bit[{}]", location, clazz.getSimpleName(), fieldName, hex, detail);
        } else {
            logger.info("--deParse @F_bit_num field{}--[{}.{}] val{} hex[{}] bit[{}]", location, clazz.getSimpleName(),
